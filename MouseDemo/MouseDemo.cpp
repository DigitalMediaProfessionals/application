...
void spc_restore_privileges(void) {
  ...
  if (!orig_uid) setgroups(orig_ngroups, orig_groups);
}
...
int get_apps_infor() {  // success -> return True, otherwise return False
  ...
  while (get_apps.good()) {
    ...
    if (cmtLine == false && str_temp[0].size() > 0) {
  ...
  /*    printf( "Number of apps: %d \n", number_of_apps);
    ...
    app_type[ii].app_num[jj].is_cam.c_str()); printf( "app_num.bin_address: %s
...
void draw_cursor_to_display(COverlayRGB &ol_screen, int xx, int yy) {
  ...
  overlay_cursor.set_mouse_cursor(0xffffffff);
...
void calculate_boxes() {
  ...
  for (int i = 0; i < MAX_APP_TYPES; i++) {
    box_area[i][0] =
        rec_area[0][0] + ((rec_area[0][2] - rec_area[0][0]) / 2 - l_box_w / 2);
    box_area[i][1] =
        rec_area[0][1] +
        (rec_area[0][3] - rec_area[0][1] - 3 * l_box_h - 2 * l_dist) / 2 +
        i * (l_box_h + l_dist);
  ...
  for (int i = 3; i < 9; i++) {
    box_area[i][0] =
        rec_area[1][0] +
        (rec_area[1][2] - rec_area[1][0] - 2 * r_box_w - r_dist_x) / 2;
    if ((i == 4) || (i == 6) || (i == 8))
      box_area[i][0] += (r_box_w + r_dist_x);
    box_area[i][1] =
        rec_area[1][1] +
        (rec_area[1][3] - rec_area[1][1] - 3 * r_box_h - 2 * r_dist_y) / 2;
  ...
  for (int i = 9; i < 11; i++) {
    ...
    if (i == 10) box_area[i][0] = rec_area[1][2] - side_dist - scr_box_w;
  ...
  for (int i = 11; i < 13; i++) {
    ...
    if (i == 12) box_area[i][0] = rec_area[0][2] - side_dist - scr_box_w;
...
void bg_on_draw() {
  ...
  int text_size = 35;
  bg_overlay.draw_ctrlBox_with_fonts(300 - 1, 20 - 1, SCREEN_W - 300 - 1,
                                     120 - 1, 0xFF000000, 0xFF000000, font_file,
                                     text, text_size, 0xFFFFFFFF, 0);

  // Draw 2 biggest boxes
  int bb_color = 0xFF16499A;
  bg_overlay.draw_ctrlBox_with_fonts(rec_area[0][0], rec_area[0][1],
                                     rec_area[0][2], rec_area[0][3], bb_color,
                                     bb_color, font_file, "", 0, 0xFFFFFFFF, 5);
  int boxname_w = 150;
  int boxname_h = 30;
  int bName_color = DMP_COLOR;
  int x_name_box =
      rec_area[0][0] + (rec_area[0][2] - rec_area[0][0]) / 2 - boxname_w / 2;
  int y_name_box = rec_area[0][1] - boxname_h / 2;
  bg_overlay.draw_ctrlBox_with_fonts(
      x_name_box, y_name_box, x_name_box + boxname_w, y_name_box + boxname_h,
      bName_color, bName_color, font_file, "CLASSIFICATION", 10, 0xFFFFFFFF, 2);

  bg_overlay.draw_ctrlBox_with_fonts(rec_area[1][0], rec_area[1][1],
                                     rec_area[1][2], rec_area[1][3], bb_color,
                                     bb_color, font_file, "", 0, 0xFFFFFFFF, 5);
  x_name_box =
      rec_area[1][0] + (rec_area[1][2] - rec_area[1][0]) / 2 - boxname_w / 2;
  y_name_box = rec_area[1][1] - boxname_h / 2;
  bg_overlay.draw_ctrlBox_with_fonts(
      x_name_box, y_name_box, x_name_box + boxname_w, y_name_box + boxname_h,
      bName_color, bName_color, font_file, "APPLICATION", 10, 0xFFFFFFFF, 2);

  // Draw quit button
  bg_overlay.draw_ctrlBox_with_fonts(1260 - 1, 0, 1280 - 1, 20 - 1, bb_color,
                                     bb_color, font_file, "X", 10, 0xFFFFFFFF,
                                     0);
  // Write DMP website
  bg_overlay.draw_ctrlBox_with_fonts(
      900, 680 - 1, 1280 - 1, 720 - 1, 0xFF000000, 0xFF000000, font_file,
      "https://www.dmprof.com", 14, 0xFFFFFFFF, 2);
}
...
void boxes_on_draw(COverlayRGB &ol_boxes) {
  if (st_changed == true) {
    ...
    for (int i = 0; i < lboxes_will_draw; i++) {
      ...
      int bg_color = DMP_COLOR;
      int otl_color = (i == (screen_mode - 3 * l_next_page) || i == hover_index)
                          ? 0xFFFFFFFF
                          : bg_color;
      ol_boxes.draw_ctrlBox_with_fonts(
          box_area[i][0], box_area[i][1], box_area[i][2], box_area[i][3],
          bg_color, otl_color, font_file, txt, 17, 0xFFFFFFFF, 5);
    }
    // Draw two button for up and down pages inside left boxes
    if (l_next_page > 0) {
      up_bt_ready = true;
      int bg_color = 0xFF16499A;
      int otl_color = ((11 == hover_index) ? 0xFFFFFFFF : DMP_COLOR);
      ol_boxes.draw_ctrlBox_with_fonts(
          box_area[11][0], box_area[11][1], box_area[11][2], box_area[11][3],
          bg_color, otl_color, font_file, "<<", 12, 0xFFFFFFFF, 3);
    } else {
      down_bt_ready = false;
    }
    if (leftover > 3) {
      down_bt_ready = true;
      int bg_color = 0xFF16499A;
      int otl_color = ((12 == hover_index) ? 0xFFFFFFFF : DMP_COLOR);
      ol_boxes.draw_ctrlBox_with_fonts(
          box_area[12][0], box_area[12][1], box_area[12][2], box_area[12][3],
          bg_color, otl_color, font_file, ">>", 12, 0xFFFFFFFF, 3);
    } else {
      down_bt_ready = false;
    }
    ...
    for (int i = 0; i < rboxes_will_draw; i++) {
      int bg_color = DMP_COLOR;
      int otl_color = (i + 3 == hover_index) ? 0xFFFFFFFF : bg_color;
      ol_boxes.draw_ctrlBox_with_fonts(
          box_area[3 + i][0], box_area[3 + i][1], box_area[3 + i][2],
          box_area[3 + i][3], bg_color, otl_color, font_file,
          app_type[screen_mode].app_num[i + 6 * next_page].name, 17, 0xFFFFFFFF,
          5);
    }
    ...
    if (next_page > 0) {
      ...
      int bg_color = 0xFF16499A;
      int otl_color = ((9 == hover_index) ? 0xFFFFFFFF : DMP_COLOR);
      ol_boxes.draw_ctrlBox_with_fonts(
          box_area[9][0], box_area[9][1], box_area[9][2], box_area[9][3],
          bg_color, otl_color, font_file, "<<", 12, 0xFFFFFFFF, 3);
    } else {
    ...
    if (leftover > 6) {
      next_bt_ready = true;
      int bg_color = 0xFF16499A;
      int otl_color = ((10 == hover_index) ? 0xFFFFFFFF : DMP_COLOR);
      ol_boxes.draw_ctrlBox_with_fonts(
          box_area[10][0], box_area[10][1], box_area[10][2], box_area[10][3],
          bg_color, otl_color, font_file, ">>", 12, 0xFFFFFFFF, 3);
    } else {
      next_bt_ready = false;
    }
...
int main(int argc, char **argv) {
  ...
  // Thread for setting up screen reload time and checking stopping signal from
  // buttons
  ...
  while (exit_code == false) {
    mouse_read(m_cur);
    int new_hover = pressed_area(m_cur);
    if (flag == true) {
    ...
    if ((m_cur.l_pressed == false) && (l_just_pressed == true)) {
      ...
      if ((pressed_zone >= 0) &&
          (pressed_zone <
           lboxes_will_draw)) {  // mouse pressed inside left boxes
        st_changed = true;
        screen_mode = pressed_zone + 3 * l_next_page;
        next_page = 0;
      } else if ((pressed_zone >= 3) &&
                 (pressed_zone <
                  rboxes_will_draw + 3)) {  // mouse pressed inside 6 big boxes
#ifdef USE_SPC_PIPE
        SPC_PIPE *p;
        int app_run = pressed_zone - 3 + 6 * next_page;
        p = spc_popen(
            app_type[screen_mode].app_num[app_run].bin_address.c_str(), NULL,
            NULL);
