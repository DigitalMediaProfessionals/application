...
int main(int argc, char** argv)
{
  ...
  if (num_images == 0) {
    ...
    if (!dmp::util::load_background_image("fpgatitle_squeezeNet.ppm"))
        return 1;
	...
	//Struct for input size fix. Original network requires a 227x227 input, but current framework does not support resize. 
	//Simply add padding...
	struct top_conv_conf& _conf = network.get_conv_layer(0);
	_conf.hw.input.w = 224; // Input Width
	_conf.hw.input.h = 224; // Input Height
	_conf.hw.run[0].conv_pad = 0x03000300; //cop0x0; // bits [7:0] = left padding, bits [15:8] = right padding, bits [23:16] = top padding, bits [31:24] = bottom padding
