...
int main(int argc, char** argv) {
  ...
  while (exit_code == -1) {
    ...
    if (conv_time_tot != 0) {
      ...
      // dmp::util::print_time_toDisplay( TEXT_XOFS, TEXT_YOFS + 2, "Fully
      // Connected (" + fc_freq + " MHz HW ACC) : ", ip_time_tot, 9999,
    ...
    if (sync_cnn_out == sync_cnn_in) {
      if (sync_cnn_out != 0) {
        ...
        dmp::util::print_image_toDisplay((SCREEN_W - IMAGE_W) / 2,
                                         (293 - 128) + 20, imgView);
        print_result(TEXT_XOFS, TEXT_YOFS - 8 - 5,
                     catrank(&networkOutput.front()), 0x88ff8800, 0x00ff0000,
                     0x00000001);
