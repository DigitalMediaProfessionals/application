...
const uint32_t class_color[] = {
    0x1F7FFF, 0x003FFF, 0xFF2600, 0xF200FF, 0xFF4C00, 0xA5FF00,
...
// DisNet Object size of person, car and truck, respectifully.
const float disnet_objsize[][3] = { {175, 55, 30}, {160, 180, 400}, {270, 245, 810}, };

static void standarize(float data[]) {
  const float mean[6] = { 3.96523491e-02, 4.47066845e-02, 2.69150662e-02, 1.67431636e+02, 1.51098052e+02, 3.21771426e+02 };
  const float std[6] = { 2.84976202e-02, 3.21873465e-02, 1.82561831e-02, 2.01949865e+01, 5.65060961e+01, 1.83260994e+02 };
  for (int i = 0; i < 6; ++i)
    data[i] = (data[i] - mean[i]) / std[i];
}

static float unstandarize(float data) {
  const float mean = 9.24339148;
  const float std = 5.32263745;
  return data * std + mean;
}

int draw_bboxes(const vector<float> &boxes, COverlayRGB &overlay) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    printf("\n");
#endif
    // handle person, car, and truck type only
    if (obj_type == 0 || obj_type == 2 || obj_type == 7) {
      int x = image_w * box[0];
      int y = image_h * box[1];
      int w = image_w * box[2];
      int h = image_h * box[3];
      ...
      float Bd = sqrt(w * w + h * h);
      int dis_obj_type = (obj_type == 0 ? 0 : (obj_type == 2 ? 1 : 2));
      float dis_in[6] = {1.0f / h, 1.0f / w, 1.0f / Bd,
        disnet_objsize[dis_obj_type][0], disnet_objsize[dis_obj_type][1], disnet_objsize[dis_obj_type][2]};
      standarize(dis_in);
      __fp16 net_in[6];
      for (int k = 0; k < 6; ++k)
        net_in[k] = (__fp16)dis_in[k];

      // Run network in HW
      memcpy(net_dis.get_network_input_addr_cpu(), net_in, 12);
      net_dis.RunNetwork();

      // Handle output from HW
      std::vector<float> tensor;
      net_dis.get_final_output(tensor);
      conv_time_tot += net_dis.get_conv_usec();
      
      float pred_dis = unstandarize(tensor[0]);
      std::stringstream ss;
      ss << std::fixed << std::setprecision(1) << pred_dis << "m";
      ...
      int lx = (x0 + x1) / 2;
      int ly = y1 + 1;
      overlay.set_line(300, 256, lx, ly, color);
      lx = int(75 + 3 * lx / 4 - 15);
      ly = int(72 + 3 * ly / 4);
      overlay.set_box_with_text(lx, ly, lx, ly, color, ss.str());
    }
  }
  return conv_time_tot;
}
