...
static inline void YUV2RGB(const unsigned char y, const unsigned char u,const unsigned char v, unsigned int* rgb)
{
  const int y2 = (int)y;
...
static void yuyv2rgb_crop(unsigned char *__restrict yuyv, unsigned int *__restrict vImg, int capture_w, int capture_h, int crop_xoffs, int crop_yoffs, int crop_w, int crop_h)
{
  assert(((capture_w & 1) == 0) && ((crop_xoffs & 1) == 0) && ((crop_w & 1) == 0));
  ...
  for (int y = crop_yoffs; y < (crop_yoffs + crop_h); y++) {
    for (int x = crop_xoffs; x < (crop_xoffs + crop_w); x += 2) {
      y0 = (unsigned char)yuyv[i]; i++;
      u = (unsigned char)yuyv[i]; i++;
      y1 = (unsigned char)yuyv[i]; i++;
      v = (unsigned char)yuyv[i]; i++;
      YUV2RGB(y0, u, v, &rgb);
      vImg[j] = rgb; j++;
      YUV2RGB(y1, u, v, &rgb);
      vImg[j] = rgb; j++;
...
int v4l2_cam_get(unsigned int * __restrict vImg, int capture_w, int capture_h, int crop_xoffs, int crop_yoffs, int crop_w, int crop_h)
{
	...
	yuyv2rgb_crop((unsigned char *)buffers[buf.index].start, vImg, capture_w, capture_h, crop_xoffs, crop_yoffs, crop_w, crop_h);
