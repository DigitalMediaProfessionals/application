...
bool load_background_image(std::string filename)
{
    ...
    if (!dfs.is_open()) {
        ...
        for (unsigned int i = 0; i < 54 - 15; i++)
            dfs.read((char*)px, 1);
        for (unsigned int j = 0; j < SCREEN_H; j++) {
            for (unsigned int i = 0; i < SCREEN_W; i++) {
                dfs.read((char*)px, 3);
                background_image[j * SCREEN_W + i] = (px[0] << 24) | (px[1] << 16) | (px[2] << 8);
            }
        }
        dfs.close();
        return true;
...
int string2bitmap(std::string s, unsigned int* b, unsigned int fcol, unsigned int bcol, int x, int y)
{
    ...
    while (s[n]) {
        ...
        unsigned int* a = b;
        for (int i = 0; i < 8; i++) {
            char bits = c < 0xA0 ? font8x8_basic[short(c)][i] : font8x8_ext_latin[short(c - 0xA0)][i];
            for (int j = 0; j < 8; j++) {
                *a = (bits & 0x1) ? fcol : ((bcol & 0x1) ? (background_image[SCREEN_W * (8 * y + i) + (8 * (x + n) + j)]) : bcol);
                a++;
                bits = bits >> 1;
            }
            a += SCREEN_W - 8;
...
int string2bitmap_xy(std::string s, unsigned int* b, unsigned int fcol, unsigned int bcol, int x, int y)
{
    ...
    while (s[n]) {
        ...
        for (int i = 0; i < 8; i++) {
            char bits = c < 0xA0 ? font8x8_basic[short(c)][i] : font8x8_ext_latin[short(c - 0xA0)][i];
            for (int j = 0; j < 8; j++) {
                *a = (bits & 0x1) ? fcol : ((bcol & 0x1) ? (background_image[SCREEN_W * (y + i) + ((x + 8 * n) + j)]) : bcol);
                a++;
                bits = bits >> 1;
            }
            a += SCREEN_W - 8;
...
int string2bitmap16x16(std::string s, unsigned int* b, unsigned int fcol, unsigned int bcol, int x, int y)
{
    ...
    while (s[n]) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            for (int j = 0; j < 16; j++) {
                *a = (bits & 0x8000) ? fcol : ((bcol & 0x1) ? (background_image[SCREEN_W * (8 * y + i) + (8 * (x + 2 * n) + j)]) : bcol);
                a++;
                bits = bits << 1;
            }
            a += SCREEN_W - 16;
...
void print16x16_toDisplay(int x, int y, std::string s, unsigned int fcol, unsigned int bcol)
{
    ...
    for (int i = 0; i < 16; i++)
        memcpy((void*)(addr + x * 8 * 4 + (y * 8 + i) * SCREEN_W * 4), (void*)(textbuf16 + i * SCREEN_W), 2 * n * 8 * 4);
}
...
void print16x24_toDisplay( int x, int y, std::string s, unsigned int fcol, unsigned int bcol)
{
    ...
    for (int i = 0; i < 24; i++)
        memcpy((void*)(addr + 2 * x * 8 * 4 + (3 * y * 8 + i) * SCREEN_W * 4), (void*)(textbuf + i * SCREEN_W), 2 * n * 8 * 4);
}
...
void print8x8_toDisplay(int x, int y, std::string s, unsigned int fcol, unsigned int bcol)
{
   ...
   int n = string2bitmap(s, textbuf8, fcol, bcol, x, y);
    for (int i = 0; i < 8; i++)
        memcpy((void*)(addr + x * 8 * 4 + (y * 8 + i) * SCREEN_W * 4), (void*)(textbuf8 + i * 8 * (SCREEN_W / 8)), n * 8 * 4);
}
...
void print24x48_toDisplay( int x, int y, std::string s, unsigned int fcol, unsigned int bcol)
{
    ...
    for (int i = 0; i < 48; i++)
        memcpy((void*)(addr + x * 8 * 4 + (y * 8 + i) * SCREEN_W * 4), (void*)(textbuf + i * SCREEN_W), 3 * n * 8 * 4);
}
...
void print_background_image_toDisplay()
{
    ...
    for (unsigned int i = 0; i < SCREEN_H; i++) { // height
        memcpy((void*)(addr + i * SCREEN_W * 4), (void*)(background_image + i * SCREEN_W), SCREEN_W * 4);
    }
}
...
void print_image_toDisplay(int x, int y, unsigned int* img, int crop_left)
{
    ...
    for (int i = 0; i < IMAGE_H; i++) {
        memcpy((void*)(addr + x * 4 + (y + i) * SCREEN_W * 4), (void*)(imgTmp + i * IMAGE_W), (IMAGE_W)*4);
    }
    */

    for (unsigned int i = 0; i < IMAGE_H; i++) {
        memcpy((void*)(addr + x * 4 + (y + i) * SCREEN_W * 4), (void*)(img + i * IMAGE_W + crop_left), (IMAGE_W - crop_left)*4);
    }
...
int string2bitmap_bbox(std::string s, unsigned int* b, unsigned int fcol, unsigned int bcol, int x, int y, unsigned int* img)
{
    ...
    while (s[n]) {
        ...
        for (int i = 0; i < 8; i++) {
            char bits = c < 0xA0 ? font8x8_basic[short(c)][i] : font8x8_ext_latin[short(c - 0xA0)][i];
            for (int j = 0; j < 8; j++) {
                //*a = (bits & 0x1) ? fcol : ((bcol & 0x1) ? (img[IMAGE_W * (y + i) + ((x + 8 * n) + j)]) : bcol);
...
//-----------------------------------------------------------------
void draw_progress_bar(unsigned int addr, int x, int y, int w, int h, int fcol, int bcol, int steps, int prog)
{
    int gap = 1;
...
void print_xy(int x, int y, std::string s, unsigned int fcol, unsigned int bcol)
{
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8 * n; j++)
            background_image[SCREEN_W * (y + i) + (x + j)] = textbuf[i * SCREEN_W + j];
    //memcpy((void*)(fbA+iomap_ddr + x*8*4 + (y*8+i)*SCREEN_W*4), (void*)(textbuf + i*8*80), n*8*4);
}
...
int string2bitmap16x24(std::string s, unsigned int* b, unsigned int fcol, unsigned int bcol, int x, int y)
{
    ...
    while (s[n]) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            for (int j = 0; j < 16; j++) {
                *a = (bits & 0x8000) ? fcol : ((bcol & 0x1) ? (background_image[SCREEN_W * (3 * 8 * y + i) + (2 * 8 * (x + n) + j)]) : bcol);
                a++;
                bits = bits << 1;
            }
            a += SCREEN_W - 16;
...
int string2bitmap24x48(std::string s, unsigned int* b, unsigned int fcol, unsigned int bcol, int x, int y)
{
    ...
    while (s[n]) {
        ...
        for (int i = 0; i < 48; i++) {
            ...
            for (int j = 0; j < 24; j++) {
                *a = (bits & 0x800000) ? fcol : ((bcol & 0x1) ? (background_image[SCREEN_W * (8 * y + i) + (8 * (x + 3 * n) + j)]) : bcol);
                a++;
                bits = bits << 1;
            }
            a += SCREEN_W - 24;
